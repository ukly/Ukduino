...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    if(100 <= dist_raw && dist_raw<=150){
      brightness = 255-(128*((dist_raw-100)/50));
      analogWrite(PIN_LED, brightness);
    } 
    else if(150 < dist_raw && dist_raw <= 200){
      brightness = 127-(127*((dist_raw-150)/50));
      analogWrite(PIN_LED, brightness);
    } 
    else if(200 < dist_raw && dist_raw <= 250){
      brightness = 0 + (127*((dist_raw-200)/50));
      analogWrite(PIN_LED, brightness);
    }
    else if(250 < dist_raw && dist_raw <= 300){
      brightness = 127 + (128*((dist_raw-250)/50));
      analogWrite(PIN_LED, brightness);
    }
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if (reading == 0.0 && last_dist_raw != 0.0) {
    reading = last_dist_raw;
  }
  else if(reading < dist_min || reading > dist_max) {
